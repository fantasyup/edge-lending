...
import "./interfaces/EdgeVaultSCI.sol";
import "./interfaces/EdgeVaultLPI.sol";
import "./interfaces/EdgeVaultLPFactoryI.sol";
import "./interfaces/EdgeVaultSCFactoryI.sol";
...
////////////////////////////////////////////////////////////////////////////////////////////
/**
EdgeControl is designed to coordinate Edge Vaults
This contract uses the OpenZeppelin contract Library to inherit functions from
  Ownable.sol
**/

contract EdgeControl is Ownable, Exponential {
    using SafeMath for uint256;

    UniswapLPOracleFactoryI public Oracle; //oracle factory contract interface
    EdgeVaultLPFactoryI public WVLPF;
    EdgeVaultSCFactoryI public WVSCF;
    ...
    mapping(address => address) public instanceLPTracker; //maps LP token address to the assets EdgeVault
    ...
    @param _WVLPF is the address for the EdgeVaultLPFactory used to produce LP Edge Vaults
    @param _WVSCF is the address for the EdgeVaultSCFactory used to produce Stable Coin Edge Vaults
    ...
    **/
    ...
    /**
    @notice viewNumLPVaults returns the number of lp vaults on the edge platform
    **/
    ...
    /**
    @notice viewNumSCVaults returns the number of stablecoin vaults on the edge platform
    **/
    ...
    /**
    @notice createNewLPVault allows the contract owner to create a new EdgeVaultLP contract for a specific LP token
    ...
    @param _lp is the address for the LP token this Edge Vault will manage
    ...
    **/
    function createNewLPVault(
        ...
        //create new Edge LP Vault
        address _EdgeVault = WVLPF.createEdgeVaultLP(_timelock, _lp, _lpName);
        //track the edge vault lp instance by the address of the LP it represents
        instanceLPTracker[_lp] = _EdgeVault;
        //add new LP Vault to the array of all LP vaults
        lpVaults.push(_EdgeVault);
        //set Edge vault address as an approved vault
        isVault[_EdgeVault] = true;
        //track vault to asset
        getAssetByVault[_EdgeVault] = _lp;
        emit NewLPVault(_EdgeVault);
    }

    function importLPVault(address _lpVault) public onlyOwner {
      require(isVault[_lpVault] == false);
        EdgeVaultLPI _vault = EdgeVaultLPI(_lpVault);
    ...
    /**
    @notice createNewSCVault allows the contract owner to create a new EdgeVaultLP contract for a specific LP token
    @param _timelock is a variable representing the number of seconds the timeWizard will prevent withdraws and borrows from a contracts(one week is 605800 seconds)
    ...
    @param _StableCoin is the address of the StableCoin this Edge Vault will manage
    **/
    function createNewSCVault(
        ...
        //create the SC Edge vault
        address _EdgeVault = WVSCF.createNewEdgeVaultSC(
            IR,
            _StableCoin,
            edgeTeam,
        ...
        //track the edge vault sc instance by the address of the stablecoin it represents
        instanceSCTracker[_StableCoin] = _EdgeVault;
        //add new SC Vault to the array of all SC vaults
        scVaults.push(_EdgeVault);
        //set Edge vault address as an approved vault
        isVault[_EdgeVault] = true;
        //track vault to asset
        getAssetByVault[_EdgeVault] = _StableCoin;
        emit NewSCVault(_EdgeVault, IR);
    }

    function importSCVault(address _scVault) public onlyOwner {
      require(isVault[_scVault] == false);

        EdgeVaultSCI _vault = EdgeVaultSCI(_scVault);
    ...
    **/
    ...
    /**
    @notice borrowSC is the function an end user will call when they wish to borrow a stablecoin from the edge platform
    ...
    **/
    function borrowSC(address _StableCoin, uint256 _amount) public {
        ...
        EdgeVaultSCI WV = EdgeVaultSCI(instanceSCTracker[_StableCoin]);
        //call _borrow function on the stablecoin edge vault
    ...
    **/
    ...
    /**
    @notice updateInterestRateModel allows the edge team to update the interest rate model for a stablecoin
    ...
    **/
    function updateInterestRateModel(
        ...
        EdgeVaultSCI WV = EdgeVaultSCI(vault);
    ...
    **/
    ...
    /**
    @notice transferEdgeTeam allows the wapr team address to be changed by the owner account
    @param _newEdge is the address of the new edge team
    **/
    function transferEdgeTeam(address _newEdge) public onlyOwner {
        uint256 numSCVaults = scVaults.length;
        edgeTeam = _newEdge;
        for (uint256 i = 0; i < numSCVaults; ++i) {
            EdgeVaultSCI WVSC = EdgeVaultSCI(scVaults[i]);
            WVSC.updateTeam(_newEdge);
        }
    }
}