...
const createNewLendingPair: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!process.env.LendingPairName || !process.env.LendingPairSymbol || !process.env.PauseGuardian
    || !process.env.CollateralAsset || !process.env.BorrowAsset || !process.env.InitialExchangeRateMantissa ||
    !process.env.ReserveFactorMantissa || !process.env.CollateralFactor || !process.env.LiquidationFee || !process.env.InterestRateModel) {
    ...
    const _borrowVars = {borrowAsset,initialExchangeRateMantissa,reserveFactorMantissa,collateralFactor,liquidationFee,interestRateModel};

    const tx = await lendingPairFactory.createLendingPairWithProxy(_lendingPairName,_lendingPairSymbol, _pauseGuardian, _collateralAsset,_borrowVars);
    
    console.log("New Lending Pair Created tx:",tx.hash)
}

export default createNewLendingPair