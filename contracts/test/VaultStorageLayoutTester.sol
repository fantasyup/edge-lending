...
contract VaultStorageLayoutTester is Vault {
    ...
    function validateStorageLayout() external pure {
        ...
        require(slot == 2 && offset == 0, "owner has changed location");
        ...
        require(slot == 4 && offset == 0, "balanceOf rate has changed location");

        assembly {
            slot := totals.slot
            offset := totals.offset
        }
        require(slot == 5 && offset == 0, "totals has changed location");
